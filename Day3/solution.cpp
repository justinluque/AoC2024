...
int computeConditionalSumOfMultiplication(const std::string &data)
{
  ...
  while (std::regex_search(searchBegin, data.cend(), matches, pattern))
  {
    ...
    else if (calculate && matches[3].matched && matches[4].matched) // Match for "mul(x,y)"
    {
...
int main()
{
  ...
  const int sumOfMultiplicationsProcessed = computeConditionalSumOfMultiplication(input);
